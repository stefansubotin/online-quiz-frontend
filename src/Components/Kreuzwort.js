        ...
        for (let i = 0; i < data.count; i++) {
            ...
            for (let j = 0; j < data.size; j++) {
                ...
                if (j + 1 < data.lines[i].start || j + 1 >= data.lines[i].start + data.lines[i].length) {
                    console.log(1)
                    line.push(<span style={{ minWidth: '50px', visibility: 'hidden' }}>S</span>);
                }
                else {
                    console.log(2)
                    let name = i + '_' + j;
                    if (data.lines[i].user == this.state.user) {
                        line.push(<input type='text' name={name} maxLength={1} style={{ minWidth: '50px', backgroundColor: {color} }} defaultValue={this.state.lines[i][j]} onChange={e => this.onChangeLine(e)} />);
                    }
                    else {
                        line.push(<input type='text' name={name} maxLength={1} style={{ minWidth: '50px', backgroundColor: {color} }} defaultValue={this.state.lines[i][j]} readOnly={true} />);
                    }
                }
            }
            //line.push(<br/>)
